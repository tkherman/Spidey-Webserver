...
for i in 4; do
	ITIME=$(./thor.py -p $i -r 2 http://student${MACHINE}.cse.nd.edu:$PORT/1k.html | grep 'TOTAL AVERAGE ELAPSED TIME: ' | cut -d ':' -f 2 | cut -d ' ' -f 2)
...
COUNT=0
for i in 4; do
	ITIME=$(./thor.py -p $i -r 2 http://student${MACHINE}.cse.nd.edu:$PORT/1m.html | grep 'TOTAL AVERAGE ELAPSED TIME: ' | cut -d ':' -f 2 | cut -d ' ' -f 2)
...
COUNT=0
for i in 4; do
	ITIME=$(./thor.py -p $i -r 2 http://student${MACHINE}.cse.nd.edu:$PORT/1g.html | grep 'TOTAL AVERAGE ELAPSED TIME: ' | cut -d ':' -f 2 | cut -d ' ' -f 2)
    echo Average time per request $i processes: $ITIME
